...
const FeedbackRatings = () => {
  ...
  const handleCancel = useCallback(
    ...
    [campData, axiosPublic]
  );

  // Filter data to show only "Paid" and "Confirmed" entries
  const filteredData = React.useMemo(() => {
    return campData ? campData.filter(camp => camp.paymentStatus === 'Paid' && camp.confirmationStatus === 'Confirmed') : [];
  }, [campData]);
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => (
         ...
         <Link to={`feedback/${row.original._id}`}> <button className="btn bg-orange-500 border-none "> Review</button></Link>
